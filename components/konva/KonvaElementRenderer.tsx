...
interface KonvaElementRendererProps {
  ...
  layerRef: React.RefObject<Konva.Layer>; // This will now be the mainLayerRef
...
const KonvaElementRenderer: React.FC<KonvaElementRendererProps> = (props) => {
  const {
      ...
      currentTool,
      isDrawingBezierPath, currentBezierPathData,
      isExtendingPathInfo, activeControlPoint, selectedBezierPointId,
      motionPathSelectionTargetElementId, editingKeyframeShapePreview,
  ...
  }, [elementsProp, stageRef, stateRef]);

  const handleKonvaMouseDownInternal = useCallback((e: Konva.KonvaEventObject<MouseEvent>) => {
    if (stateRef.current.currentTool === 'select' && e.target.draggable() && stageRef.current && e.evt.button === 0) {
    ...
    if (stage) {
        ...
        if(currentCanvasCursor === CURSOR_GRAB || currentCanvasCursor === CURSOR_POINTER || currentCanvasCursor === CURSOR_PEN_ADD) {
             const defaultCursor = stateRef.current.currentTool === 'pencil' || (stateRef.current.currentTool === 'bezierPath' && (stateRef.current.isDrawingBezierPath || stateRef.current.isExtendingPathInfo))
                                ? 'crosshair'
                                : (stateRef.current.currentTool === 'bezierPath' ? 'crosshair' : 'default');
  ...
  const mapElementToKonvaShape = useCallback((element: SVGElementData): Konva.Shape | Konva.Group | null => {
    ...
    else if (element.type === 'text') konvaShape = new Konva.Text({ ...commonConfig, padding: 0 });
    ...
    if (konvaShape) {
      ...
      if (stateRef.current.currentTool === 'select' && element.id !== artboard.id) {
    ...
    return konvaShape;
      ...
      selectedElementIdProp, artboard.id, stateRef,
      handleKonvaPointerDown,
  ...
  useEffect(() => {
    ...
    const updateOrCreateNode = (elementData: SVGElementData, parentKonvaGroup: Konva.Group): Konva.Node | null => {
        ...
        if (konvaNode instanceof Konva.Path || konvaNode instanceof Konva.Rect || konvaNode instanceof Konva.Ellipse || konvaNode instanceof Konva.Text || konvaNode instanceof Konva.Image ) {
          konvaNode.hitStrokeWidth(PATH_HITBOX_EXTRA_WIDTH / stageZoom);
        }

        let directSetAttrs: Konva.ShapeConfig = { name: displayElement.name || displayElement.type }; // Changed type to ShapeConfig
        ...
        const propsForFrame: Konva.ShapeConfig = { ...directSetAttrs }; // Use ShapeConfig here
        ...
        const modelSkewX = Number.isFinite(displayElement.skewX) ? displayElement.skewX : DEFAULT_SKEW_X;
        const modelSkewY = Number.isFinite(displayElement.skewY) ? displayElement.skewY : DEFAULT_SKEW_Y;

        const transformPropsToSet: Konva.NodeConfig = {
            rotation: modelRotation, scaleX: modelScale, scaleY: modelScale,
            skewX: modelSkewX, skewY: modelSkewY,
        };
        ...
        if (displayElement.type === 'rect') {
            ...
            propsForFrame.padding = 0;

            propsForFrame.width = typeof textEl.width === 'number' ? textEl.width : undefined;
            propsForFrame.height = typeof textEl.height === 'number' ? textEl.height : undefined;
        ...
        try {
            ...
            if (isNodeBeingDirectlyDragged || isNodeBeingTransformed) {
                const { x, y, offsetX, offsetY, rotation, scaleX, scaleY, skewX, skewY, ...nonTransformAttrs } = propsForFrame;
                konvaNode.setAttrs(nonTransformAttrs);
            } else {
                konvaNode.setAttrs(propsForFrame); // No need to cast if propsForFrame is already ShapeConfig
            }
        } catch (e) {
    ...
    elementNodeMapRef.current.forEach((node, id) => {
      ...
      selectedElementIdProp, stateRef, // Use stateRef directly
      editingKeyframeShapePreview, mapElementToKonvaShape,
      motionPathSelectionTargetElementId, stageRef, layerRef,
      artboardGroupRef, elementNodeMapRef, imageNodeCacheRef,
      transformerRef, nodeIsBeingDirectlyDraggedRef,
      onCanvasTextEditor, currentTool // Added currentTool
    ]
  );
