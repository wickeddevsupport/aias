...
import React, { useRef, useEffect, useLayoutEffect, useCallback, useContext, useState } from 'react';
...
const KonvaCanvasContainer: React.FC<KonvaCanvasContainerProps> = ({
  ...
  const handleKonvaTransformEndInternal = useCallback((e: Konva.KonvaEventObject<Event>) => {
    ...
    if (originalModelElement.type === 'rect' || originalModelElement.type === 'image') {
        ...
        let baseWidthForScaling = (originalModelElement as TextElementProps).width;
        if (baseWidthForScaling === undefined) {
            const knw = textNode.width();
            baseWidthForScaling = typeof knw === 'number' ? knw : textNode.getTextWidth();
        }

        let baseHeightForScaling = (originalModelElement as TextElementProps).height;
        if (baseHeightForScaling === undefined) {
            const knh = textNode.height();
            baseHeightForScaling = typeof knh === 'number' ? knh : textNode.getTextHeight();
        }
  ...
  const handleKonvaPointerDownKonva = useCallback((e: Konva.KonvaEventObject<PointerEvent>, elementIdFromNodeArgument: string) => {
    if (stateRef.current.onCanvasTextEditor?.isVisible) {
        ...
        if (!clickedOnTextEditor) {
             const editor = stateRef.current.onCanvasTextEditor!;
             const textElementModel = stateRef.current.elements.find(el => el.id === editor.elementId) as TextElementProps | undefined;
    ...
    const {
        ...
        elements: currentElementsFromState,
  ...
  const handleBezierPointPointerDownKonva = useCallback((e: Konva.KonvaEventObject<PointerEvent>, pathId: string, pointId: string, handleType: 'anchor' | 'h1' | 'h2') => {
    ...
    if (localIsDrawingBezierPath && localCurrentBezierPathData?.id === pathId) {
        ...
        pathDataForOp = { ...animatedPathElement, ...rawPathElementFromState };
    } else {
  ...
  useLayoutEffect(() => {
    ...
    const updateDrawingSurface = () => {
        ...
        if (transformerRef.current) {
            transformerRef.current.borderStrokeWidth(strokeScaleFactor * 1.0);
            transformerRef.current.anchorSize(6 * strokeScaleFactor);
            transformerRef.current.rotateAnchorOffset(15 * strokeScaleFactor);
        }
        layer.batchDraw();
  ...
  useEffect(() => {
    ...
    if (isBezierPointEditing) {
        ...
        if (nodeToAttach && nodeToAttach.getLayer() && nodeToAttach.getStage() && nodeToAttach.visible()) {
            ...
            const currentStageZoom = stageScale;
            const scaleFactorForTransformer = currentStageZoom !== 0 ? 1 / currentStageZoom : 1;
            tr.borderStrokeWidth(scaleFactorForTransformer * 1.0);
            tr.anchorSize(6 * scaleFactorForTransformer);
            tr.rotateAnchorOffset(15 * scaleFactorForTransformer);
  ...
  return (
    <div className="w-full h-full relative">
      ...
      <div ref={containerRef} className="w-full h-full bg-dark-bg-primary overflow-hidden touch-none select-none" style={{ cursor: defaultToolCursor }}>
