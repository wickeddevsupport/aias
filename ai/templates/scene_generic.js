...
export function buildGenericScene(plan, artboard) {
  ...
  actions.push(
    ...
    withParent(addRect(skyId, 0, 0, width, height, resolveBackgroundFill(plan, palette, `${skyId}-grad`, 110)), sceneId),
    withParent(addRect(groundId, 0, height * 0.7, width, height * 0.3, palette[2] || '#22c55e'), sceneId),
    withParent(addCircle(accentId, width * 0.7, height * 0.3, Math.min(width, height) * 0.08, palette[3] || '#facc15'), sceneId)
  );
  actions.push(
    addKeyframe(accentId, 'y', 0, height * 0.32),
    addKeyframe(accentId, plan.duration || 4, height * 0.26, 'ease-in-out')
  );
  return { summary: 'Generated a simple scene with sky and ground.', actions, rootId: sceneId };
}
