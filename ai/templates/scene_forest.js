...
export function buildForestScene(plan, artboard) {
  ...
  actions.push(
    ...
    addRect(groundId, 0, height * 0.7, width, height * 0.3, palette[1] || '#166534')
  );
  ...
  for (let i = 0; i < treeCount; i += 1) {
    ...
    actions.push(
      addRect(trunkId, x - trunkW / 2, baseY - trunkH, trunkW, trunkH, '#7c2d12'),
      addCircle(crownId, x, baseY - trunkH, Math.min(width, height) * 0.08, palette[2] || '#16a34a')
    );
    actions.push(
  ...
  return { summary: 'Built a forest scene with layered trees.', actions };
}
